...
with st.sidebar:
    ...
    st.header("Vishwa Prerepa | Class Monitor")
...
tab1, tab2, tab3, tab4 = st.tabs(["Campus-connect", "Academics", "Bio-Data", "Skills"])


#tab 1 Campus-connect data
with tab1:
    st.header("🙌Welcome to Campus-Connect Program..!")
    st.write("Please enter your data to connect with us")
    ...
    a1.container(border=True).write("© 2026 Campus Connect")
...
with tab2:
    ...
    with c3:
        st.write("")
        st.write("Mathematics IA:")
        st.write("")
        st.write("Mathematics IB:")
        st.write("")
        st.write("Physics:")
        st.write("")
        st.write("Chemistry:")
...
with tab3:
        ...
        with b1:
            st.write("Full Name -")
            st.write("Date Of Birth (dd/mm/yy) -")
            st.write("Course -")
            st.write("Nationality -")
        st.divider()
        with b2:
            st.write("Vishwa Prerepa")
            st.write("28/09/2009")
            st.write("MPC")
            st.write("American")
        ...
        st.markdown("- 🏋️‍♂️I go to the Gym atleast three time a week.")
        st.markdown("- 🧊In my free time, I solve Rubik's Cube.")
        st.markdown("- 🎧I like listening to music on a regular basis.")
        st.markdown("- 🎮Sometimes I play video games on my console.")

        st.divider()

        st.subheader("🎯Goals")
        
        st.markdown("- Master Modern Tech🖥️: Deepen my expertise in core programming languages and explore the practical foundations of Artificial Intelligence.")

        st.markdown("- Academic Excellence👨‍🎓: Maintain a high standard of academic success and continuously push myself to the top of my class.")

        st.markdown("- Bridging Logic & Code👨‍💻: Apply the advanced mathematical and scientific concepts from my MPC studies directly to complex coding problems.")

        st.markdown("- Impactful Development💥: Transition from building minor tools to developing scalable, real-world applications that solve actual user problems.")
